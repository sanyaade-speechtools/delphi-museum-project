 ...
 *
 */
package edu.berkeley.delphi.categorizer;
...
import edu.berkeley.delphi.utils.NGram;
import edu.berkeley.delphi.utils.NGramStack;
...
public class Categorizer {
			...
			if(compileUnmatchedUsage) {
				...
				for(int i=1; i<nCols; i++ ) {
					...
					currFilename = basefilename
	    						+"_"+(colDumpInfo[i].getName().replaceAll("\\W", "_"))+"_Usage.txt";
	...
	private void categorizeObjectForFacet( int id, String source, ArrayList<String> allStrings,
		...
		ArrayList<Pair<String,ArrayList<String>>> tokenList
				= ConfigStringUtils.prepareSourceTokens( source, colInfo );
		...
		for( Pair<String,ArrayList<String>> pair:tokenList ) {
			...
			if( node != null ) {
				...
				ArrayList<String> words = pair.getSecond();
				if(words.size()>1) {
	...
	private boolean checkAllFacetsForToken(int id, String token,
		...
		for( DumpColumnConfigInfo.MineInfoForColumn mineInfo:colInfo.getFacetsToMine() ) {
			TaxoNode node = facetMapHashTree.FindNodeByHook(mineInfo.getFacetName(),token);
			if( node == null )
				continue;
			debug(3, "Obj:"+id+" matched concept: ["+
					mineInfo.getFacetName()+":"+node.displayName+"] on "
						+(isNGram?"NGram: ":"token: ")+token);
			// We have a candidate here. Check for the exclusions
			if( lineContainsExclusions( node.exclset, allStrings )) {
				debug(3, "Obj:"+id+" excluding matched concept: ["+
						mineInfo.getFacetName()+":"+node.displayName+"]");
				continue;
			}
			foundMatch = true;
			// We discount for nGrams
			// 1-((1-prop)*disc) is applied to confidence.
			float rel = mineInfo.getReliability();
			if(isNGram)
				rel *= 1-((1-nGramProportion)
						*facetMapHashTree.GetNGramDiscountForFacet(mineInfo.getFacetName()));
			// We have a TaxoNode match! Update matches with this node
			updateMatches( matchedCats, rel, node);
			// Since we got a match, we may stop considering the other facets for this token.
			if( mineInfo.isFilterTokenOnMatch() )
				break;
		}
		return foundMatch;
	}
