...
-->
					<p class="smaller">Sets are collections of objects made by users.
								 You can view sets that contain this object or add this object to
								 one of your own sets.</p>
					{if !$currentUser_loggedIn}
						<p><b>You must be signed in to use sets functionality.</b></p>
					{else}
	          {if $containingSets}
							...
							{section name=set loop=$otherSets}
								<div class="detail_publicSetlet">
									...
									<div class="detail_publicSetletDetails">
										...
										<span class="smaller">Created by
											<a href="{$wwwroot}/modules/auth/profile.php?uid={$otherSets[set].owner_id}">
												 {$otherSets[set].owner_name}</a>
										</span>
									</div>
								</div>
							{/section}
							{if $moreSetsLink}
								<p class="smaller" id="detail_publicSetletAllLink">
									<a href="#">See all sets containing this object</a></p>
							{else}
								<br class="clear"/>
							{/if}
						{/if}
						...
						<div class="detail_personalSetlet smaller">
							...
							<div class="detail_personalSetletDetails">
								{if $personalSets[set].contains_obj}
									...
									<a href="{$wwwroot}/modules/sets/api_addToSet.php?obj_id={$id}&amp;set_id={$personalSets[set].set_id}"
											id="addToSet{$personalSets[set].set_id}" class="ajaxLink"
											title="Remove object from this set">[remove]</a>
								{else}
									{$personalSets[set].set_name}<br/>
									<a href="{$wwwroot}/modules/sets/api_addToSet.php?obj_id={$id}&amp;set_id={$personalSets[set].set_id}"
											id="addToSet{$personalSets[set].set_id}" class="ajaxLink"
											title="Add object to this set">[add to set]</a>
									<img src="{$themeroot}/images/indicator_s.gif" style="display:none;"
											id="addToSet{$personalSets[set].set_id}Indicator" alt="Spinning indicator"/>
								{/if}
							</div>
						</div>
						{/section}
						<p class="smaller" id="detail_personalSetletLinks">
							<a href="{$wwwroot}/modules/sets/api_createNewSet.php?obj_id={$id}&amp;owner_id={$currentUser_id}"
									class="ajaxLink" id="" title="Create a new set with this object">
										Create a new set with this object</a><br/>
					...
					<div id="detail_tagsTab">
						...
						{if !$currentUser_loggedIn}
							<p><b>You must be signed in to use tags functionality.</b></p>
						{else}
