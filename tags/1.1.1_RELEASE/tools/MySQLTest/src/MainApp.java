...
public class MainApp {
	...
	private String queryTextDefault = "show tables";
	private String serverIPTextDefault =
		"jdbc:mysql://sierra.ist.berkeley.edu:3031/pahma_dev?user=devbrowse&password=xxx";
	...
	private JTextField getServer_ip_tf() {
		if (server_ip_tf == null) {
			server_ip_tf = new JTextField( serverIPTextDefault );
		...
		if (ConnectBtn == null) {
			...
			ConnectBtn.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent evt) {
					...
					if( openDBConn(server_ip_tf.getText()) )
						jTextArea.append("Established connection to the MySQL DB.");
			}
		});
		}
		return ConnectBtn;
	...
	private JTextArea getQueryTextArea() {
		if (queryTextArea == null) {
			queryTextArea = new JTextArea( queryTextDefault );
	...
	private JButton getRunQBtn() {
		if (runQBtn == null) {
			...
			runQBtn.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent evt) {
					runDBStmt(queryTextArea.getText());
				}
			});
		}
		return runQBtn;
	...
	private boolean openDBConn( String connectionUrl ) {
	...
	private void runDBStmt(String statement) {
		try {
			// Create and execute an SQL statement that returns some data.
			//String SQL = "sp_help";
			jdbcStmt = jdbcCon.createStatement();
			jTextArea.setText("Executing statement...");
			jdbcResultSet = jdbcStmt.executeQuery(statement);
